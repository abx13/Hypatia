...
graines = [1, 10, 11]
date = ["2022-06-15-1943", "2022-06-15-2231", "2022-06-16-0128"]
mbps = 5
tps_simu = 120

doss_graine = "sauvegardes/svgde_"
#doss = "/run_loaded_tm_pairing_"+str(mbps)+"_Mbps_for_"+str(tps_simu)+"s_with_tcp_algorithm_free_one_only_over_isls/logs_ns3/"
interdoss="/logs_ns3/"

colours=['g','b','r']
fig,axes=plt.subplots(3,1, figsize=(16,9), dpi=80, facecolor="w", edgecolor='k')
label=['graine 1', 'graine 10', 'graine 11']

 #couleurs différentes pour chaque graine

for index in range (len(graines)) : 
    #print(index)
    #print(date[index])
    #print(graines[index])

    #print("../../"+doss_graine+date[index]+"_"+str(graines[index]))

    dossiers=sorted([runsdoss for doss in os.listdir("../../"+doss_graine+date[index]+"_"+str(graines[index]))  if os.path.isdir(runsdoss:= "../../"+doss_graine+date[index]+"_"+str(graines[index])+"/"+doss) and str(tps_simu) in doss and "isls2" in doss])
    
    for doss in dossiers:
        #print(doss)
        fics=sorted([fic for fic in os.listdir("../../"+doss_graine+date[index]+"_"+str(graines[index])+"/"+doss+interdoss) if os.path.isfile("../../"+doss_graine+date[index]+"_"+str(graines[index])+"/"+doss+interdoss+fic) and "tcp_flow_" in fic])
        if fics:
            ...
            print(colours)
            print(label)
            axes[0].plot(t_cwnds,data_cwnds,colours[index], label=label[index])
            ...
            axes[1].plot(t_rtts,data_rtts,colours[index], label=label[index])
            ...
            axes[2].plot(t_prgs,data_prgs,colours[index], label=label[index])
